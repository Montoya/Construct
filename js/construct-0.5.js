...
function containerSel(n){ 
  ...
  if( n == -1 ) {
    ...
    $('#construct .container.selected').removeClass('selected').prevAll('.container').slice($(this).length - 1, 1).addClass('selected');
    ...
    $('#construct .container.selected').removeClass('selected').nextAll('.container').slice(0,1).addClass('selected');
...
function containerDelete() {
  // remove the selected container from the DOM (+ <hr>) and select the next one
  if( $('#construct .container.selected').next().is('hr') ){
    $('#construct .container.selected').next().remove();
  }
...
function columnDelete() {
  ...
  if( $('#construct .container.selected .column.selected').size() < 1 ) {
    ...
    $('#construct .container.selected .column:last').addClass('selected').addClass('last');
  }
...
$(document).ready(function(){
  ...
  $(document).keydown(function(e){
    ...
    switch(key) {
      ...
      case 70: // F
        ...
        break;
      case 67: // C
        lorem( $('.container.selected .column.selected') );
        return false;
        break;
      case 86: // V
        $('.container.selected .column.selected').empty().append('&nbsp;');
        return false;
        break;
      case 90: // Z
        toggleHR( $('.container.selected') );
        return false;
        break;
      case 88: // X
        return false;
  ...
  $('.construct-hideLink a').click(function(){
  ...
  $('#construct-preview').click(function() {
    $('#construct').removeClass('edit').addClass('preview');
    $(this).css('display','none');
    $('#construct-edit').css('display','inline');
    return false;
  });
  
  $('#construct-edit').click(function() {
    $('#construct').removeClass('preview').addClass('edit');
    $(this).css('display','none');
    $('#construct-preview').css('display','inline');
    return false;
  });
